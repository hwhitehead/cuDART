...
where here :code:`nx`, :code:`ny` and :code:`nz` are the number of cells in each cardinal direction and in the relativistic case
the fourth index spans emissivity and velocity in each of the cardinal directions. 

It is worth noting that Python genarlly does not ensure its arrays have consistent C-style ordering. It is up to the user to ensure
that the saved :code:`.npy` data has true C-style array ordering and has not been reordered through stride meta-data alteration. C-style ordering
can be forced in Python using :code:`data = np.array(data, order="C")`.

Input data may be unlabelled, or labeled:

* Unlabelled mode: each simulation snapshot is stored a single :code:`.npy` file
* Labelled mode: each simulation snapshot is stored within a directory, containing multiple :code:`.npy` files for each sub-domian, and a header file :code:`header.txt` which contains additional information
...
--------------------------
While useful for manipulation in Python, :code:`.npy` files are not the standard outputs from hydrodynamic simulations codes. To support conversion 

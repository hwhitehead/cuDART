...
The cuDART codebase can be split into two halves, the backend which is written in C++/CUDA and runs on both the CPU and GPU, and the frontend which is written in Python and runs only on the CPU. 
